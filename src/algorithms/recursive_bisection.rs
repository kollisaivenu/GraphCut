...
pub(crate) type Point2D = SVector<f64, 2>;
pub(crate) type Point3D = SVector<f64, 3>;
pub(crate) type PointND<const D: usize> = SVector<f64, D>;
pub(crate) type Matrix<const D: usize> = SMatrix<f64, D, D>;

// Axis-aligned bounding box.
#[derive(Debug, Clone)]
struct BoundingBox<const D: usize> {
    pub p_min: PointND<D>,
    pub p_max: PointND<D>,
}
...
impl<const D: usize> BoundingBox<D> {
    ...
    // contains all given points and is aligned with the axes.
...
// Initializes a new array of a fixed size `N` by calling a closure `f` for each element index.
...
// Creates a new array by applying a mapping function to each element of a mutable input array.
...
// Creates two new arrays from the input array using a map function on the elements of the input array.
...
// Creates two new arrays by using a map function on the borrowed elements of the input array.
...
// Stores the points, weights of those points, and the parttitions those points belong to.
...
// Performs recursive splitting of points until the desired number of partitions is obtained
...
where
    P: IntoParallelIterator<Item = PointND<D>>,
    P::Iter: IndexedParallelIterator + Clone,
    ...
    W::Iter: IndexedParallelIterator,
{
    ...
    let atomic_partition = as_atomic(partition);
...
// # Panics
//
// Panics on platforms where `usize` and `AtomicUsize` do not have the same
...
// Trait alias for values accepted as weights by [Rcb].
trait RcbWeight
where
